...
class WorldWriter():
  ...
  def __call__(self):
    ...
    r_lower = -self.cyl_radius+4
    r_upper = self.r_shift - self.cyl_radius+4
    ...
    max_limit=kkk+5
    while kkk<=7:
      self._create_cyl(r_lower, kkk, 0, 0, 0, 0, radius=self.cyl_radius, rgb=wall_rgb)
      self._create_cyl(r_upper, kkk, 0, 0, 0, 0, radius=self.cyl_radius, rgb=wall_rgb)
      kkk += self.cyl_radius * 2
    print("kkk:",kkk)
    print("max_limit:",c_upper)
