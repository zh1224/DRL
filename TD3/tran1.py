...
class TD3(object):
    ...
    def train(
        ...
        self.writer.add_scalar("Critic-Loss", c_av_loss / iterations, self.iter_count)
        self.writer.add_scalar("Actor-Loss", a_av_loss / iterations, self.iter_count)
...
environment_dim = 20
robot_dim = 4
env = GazeboEnv("TD2_world.launch", environment_dim)
time.sleep(5)
...
while timestep < max_timesteps:
    ...
    if done:
        if(env.choice):
            env.test+=1
            print("success rate",env.succes,env.test,1.0*env.succes/env.test)
            network.writer.add_scalar("Success Rate",  1.0*env.succes/env.test,episode_num)
            if(episode_timesteps!=0):
                network.writer.add_scalar("Rward", episode_reward/episode_timesteps, episode_num)
        if timestep != 0:
            network.train(
                replay_buffer,
                episode_timesteps,
        ...
        if(np.random.uniform(0, 1) > epsilon):
            ...
            env.choice=False 
        if(1.0*env.succes/env.test>0.3):
            epsilon=0
        else:  
