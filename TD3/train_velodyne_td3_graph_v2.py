...
from velodyne_env_graph_v2 import GazeboEnv
...
class TD3(object):
    ...
    def get_action(self, state_nfv: np.ndarray):
        """
        Parameters
        ----------
        state_nfv : (1344,) numpy array   # 已经是 NFV
        Returns
        -------
        action : 1-D numpy array          # [v, ω]（或你的 act_dim）
        """
     #   device = self.device              # e.g. torch.device('cpu')

        # 1) numpy → torch
        state_t = torch.from_numpy(state_nfv).float().unsqueeze(0).to(device)  # (1,1344)

        # 2) 推理阶段务必关闭梯度
        with torch.no_grad():
            action_t = self.actor(state_t)         # (1, act_dim) Tensor, no grad
...
if __name__ == '__main__':
    ...
    logdir = f"./graph//max_speed_1_collsion_dist_0.45_k_250_threshold_0.45_1/reward+{scale_to_goal}+{obs_scale}+{logtest}+{test}+{up_speed}+{smoothness_scale}+{acceleration_scale}+{ounoise_decay_step}+{step_penalty}"
    processes=[]
    params={'n_workers':1,
            'n_loggers':2,
            }
