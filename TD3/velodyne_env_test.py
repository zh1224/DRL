...
GOAL_REACHED_DIST = 0.9
COLLISION_DIST = 0.36  #小范围需要改称0.15，原来是0.35  
TIME_DELTA = 0.1
...
def check_pos(x, y):
    ...
    #             goal_ok = False
    #             break
    if x<-6.3 or x>6.3 or y<-6.3 or y>6.3:#检查边界
        goal_ok = False
    return goal_ok
...
class GazeboEnv:
    ...
    def __init__(self, launchfile, environment_dim,roscoreip,gazebo_port,world_file):
        ...
        command = f'source /home/l/DRL-robot-navigation-main-LZH-test/DRL-robot-navigation-main/catkin_ws/devel/setup.bash && roslaunch {fullpath} world_name:={world_file}'
        subprocess.Popen(['bash', '-c', command])
        time.sleep(25)#这里要注意，如果出现pose无法找到，就是这个时间间隔过小
    ...
    def reset(self):
        ...
        #     position_ok = check_pos(x, y)
            # if  x>-1.5 and x<6.5 and y>-0.5 and y<2.5:
            #     position_ok=False
            # if  x>-3 and x<3 and y>2.5 and y<5.5:
            #     position_ok=False
