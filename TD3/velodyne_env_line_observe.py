...
def convex_hull_points(pts: np.ndarray) -> np.ndarray:
            ...
            return np.asarray(lower[:-1] + upper[:-1])    # (m,2)


def cluster_to_edges(cluster: np.ndarray):
        MIN_EDGE_LEN = 0.1
        MIN_VALID_D  = 0.3     # 离原点 <5 cm 直接丢弃     
        V = convex_hull_points(cluster)                  # (m,2)
        m = V.shape[0]
        edges = []
        for i in range(m):
            p1, p2 = V[i], V[(i + 1) % m]
            edge   = p2 - p1
            if np.linalg.norm(edge) < MIN_EDGE_LEN:
                continue  
            ...
            d = - n @ p1   
            d=abs(d)                              # 有向垂直距
            if d <= MIN_VALID_D:            # ★ 背向或离原点过近 ⇒ 丢弃
                continue

            edges.append(dict(d=d, n=n, p=p1, q=p2))
        return edges
...
class GazeboEnv:
    ...
    def __init__(self, launchfile, environment_dim,roscoreip,gazebo_port,t,k):
        ...
        self.K = k
        self.active_edges = [
            dict(d=10.0, n=np.zeros(2), p=None, q=None) for _ in range(self.K)
        ]
        # ① 距离部分：全部设成 10.0 代表“默认很远”
        d_init = np.full(self.K, 10.0, dtype=np.float32)

        # ② 角度部分：全部设成 0（朝正前）或其它任意占位角
        theta_init = np.zeros(self.K, dtype=np.float32)

        # ③ 拼成 edge_feat = [d₁…d_K, θ₁…θ_K]  共 2K 维
        self.edge_feat = np.concatenate([d_init, theta_init])   # shape = (2*K,)
        #self.edge_feat = np.concatenate([[10.0, 0.0, 0.0] for _ in range(self.K)])  # 12 维
...
# ── 可选：若装得到 SciPy → 用官方 ConvexHull 快很多

        # Graham-Scan / Andrew Monotone-Chain，纯 NumPy
    ...
    def velodyne_callback(self, v):
      ...
      #          print(data[i][0],data[i])
                ...
                #        print(obs_position_x,obs_position_y)
                        ...
                        break
        # ---------- D. 若本帧无障碍点 ----------
        local_xy = np.array([(pt[0], pt[1]) for pt in data if pt[2] > -0.2])
        if local_xy.shape[0] == 0:
            ...
            # ③ 拼成 edge_feat = [d₁…d_K, θ₁…θ_K]  共 2K 维
            self.edge_feat = np.concatenate([d_init, theta_init])   # shape = (2*K,)
          #  self.edge_feat = np.concatenate([[10.0, 0.0, 0.0] for _ in range(self.K)]) 
            return
        
         # ---------- E. 点 → 聚类 ----------
        db = DBSCAN(eps=0.25, min_samples=3).fit(local_xy)
        all_edges = []
        for lbl in set(db.labels_):
            if lbl == -1:
                continue
            cluster = local_xy[db.labels_ == lbl]
            if cluster.shape[0] < 3:
                continue
            edges = cluster_to_edges(cluster)                 # 获得该面所有边
            edges = sorted(edges, key=lambda e: e['d'])       # 按距离排序
            all_edges.extend(edges[:2])                       # 只保留前 1~2 条
         # ---------- F. 选最近 K 条边 ----------
       
        all_edges = sorted(all_edges, key=lambda e: e['d'])
        active = all_edges[:self.K]
        while len(active) < self.K:                      # 零填充
            active.append(dict(d=10.0, n=np.zeros(2), p=None, q=None))

        # ---------- G. 写成员变量 ----------
        self.active_edges = active                 # 供可视化 / 约束
       # self.edge_feat = np.concatenate([[e['d'], *e['n']] for e in active])  # (3K,)
       # velodyne_callback 内
        edges_sorted = sorted(active, key=lambda e: e['d'])[:self.K]

        #   1) 距离
        d_vec = np.asarray([e['d'] for e in edges_sorted], np.float32)

        #   2) 方位角   θ ∈ (−π, π]
        theta = np.asarray([np.arctan2(e['n'][1], e['n'][0])
                            for e in edges_sorted], np.float32)
        ...
        #   4) 拼成观测
        self.edge_feat = np.concatenate([d_vec, theta])   # 2K 维
       # print("velodyne:",self.velodyne_data)
      #  print("edge_feat:",self.edge_feat)
    ...
    def step(self, last_state,action,omega):
        ...
        # 原始数据 v_state是list
        # ---------- 原 edge_feat = [d1…dK, θ1…θK] ----------
        v_raw = np.asarray(self.edge_feat, dtype=np.float32)   # shape (2K,)
        ...
        # ---------- 归一化距离，保留符号 ----------
        # 0~10 m → 0~1 ;   -10~0 m → -1~0
        d_norm = np.clip(d_raw / 10.0, -1.0, 1.0)

        # ---------- 重新拼回 ----------
        v_state = np.concatenate([d_norm, theta], dtype=np.float32)   # (2K,)
         ...
         #   print("reward_goal:",reward)
          ...
          #  print("goal_reard:",reward)
           ...
           # reward=0.0
            ...
            reward-=self.obs_scale* 1.0/ (1 + np.exp(f * (distance - threshold)))
            # if  distance<0.7:
