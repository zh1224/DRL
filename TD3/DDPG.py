...
class Critic(nn.Module):
    ...
    def forward(self, s, a):
        ...
        s1 = F.relu(self.layer_2_s(s1) + self.layer_2_a(a))  # 结合状态和动作层
        q = self.layer_3(s1)  # 输出 Q 值
        return q
...
class DDPG(object):
   ...
   #     print("11111111111111111111111111111111111111111111111111111111train parameter:",batch_size,discount)
        ...
        for it in range(iterations):
            ...
            # q_PF = -((1 - torch.cos(action[:, 0] - U_s)) + (1 - torch.cos(action[:, 1] - omega)))
            # q_PF = q_PF.view(-1, 1)
           #print(f"q_PF shape: {q_PF.shape}")
            #print("action:",action)
            #print("q_PF",action[:,0],U_s,action[:,1],omega,q_PF)
          #  F_C_Ei = F.mse_loss(current_Q1, q_PF) 
            # Calculate the loss between the current Q value and the target Q value
            critic_sub_loss = F.mse_loss(current_Q1, target_Q) 
    ...
    # 将它们沿着列方向（dim=1）拼接成 [40, 2] 的张量
              #  combined_action = torch.cat((U_s, omega), dim=1)
                ...
                actor_grad = self.critic(state, predicted_action)
                q_PF = ((1 - torch.cos(predicted_action[:, 0] - U_s)) + (1 - torch.cos(predicted_action[:, 1] - omega)))
                #actor_grad=actor_grad.detach()
                actor_grad_combined = (-(1 - Lamda) * actor_grad + Lamda * q_PF).mean()  # 结合APF和DDPG，并转换为标量
...
def worker(t,network,counter_epoch,counter_epoch_success,param,success_queue,reward_queue,Critic_Loss_queue,Actor_Loss_queue,Av_Q_queue,Max_Q_queue,succes_all,counter_epoch_all,timestep_worker,expl_noise_episode,reward_all,win,all_step,speed,scale_to_goal,beta,Lamda,beta_Lamda_decay,obs_scale,timestep_episode,logtest,up_speed,smoothness_scale,acceleration_scale,ounoise_decay_step,counter_epoch_collision,collision_queue,counter_epoch_timeout,timeout_queue,step_penalty,W_speed,test):
   # writer = SummaryWriter(log_dir=f"runs/1worker_, reward14+nopre") 
    ...
    if not os.path.exists("./results"):
        os.makedirs("./results")
    if save_model and not os.path.exists("./pytorch_models/DDPG"):
        os.makedirs("./pytorch_models/DDPG")
    ...
    while timestep < max_timesteps:
        ...
        if done:
                    ...
                    if timesteps_since_eval >= eval_freq:
                        ...
                        # )
                        network.save(file_name+str(test), directory="./pytorch_models/DDPG")
...
if __name__ == '__main__':
    ...
    action_dim = 2
    max_action = 1
    MasterNode = DDPG(state_dim, action_dim, max_action) #A
    ...
    logdir = f"./DDPG/max_speed_1_collsion_dist_0.45_k_250_threshold_0.45_1/reward+{scale_to_goal}+{obs_scale}+{logtest}+{test}+{up_speed}+{smoothness_scale}+{acceleration_scale}+{ounoise_decay_step}+{step_penalty}"
    processes=[]
    params={'n_workers':20,
            'n_loggers':2,
            }
