...
class Actor(nn.Module):
    ...
    def sample(self, state):
        """
        使用 reparameterization trick 来采样动作，并且返回对应的 log_prob
        同时做 tanh-squash 修正
        """
        ...
        # 使用 rsample() 保证梯度可回传
        e = dist.rsample()  # e ~ N(0,1)
        # 未squash的动作
        action_unbound = e
        # log_prob（未经过tanh）
        log_prob_unbound = dist.log_prob(action_unbound)
        # 对同一 batch，动作维度求和
        log_prob_unbound = torch.sum(log_prob_unbound, dim=1, keepdim=True)

        # --- tanh squash ---
        action = torch.tanh(action_unbound)
        # 这里的 self.max_action 是动作上限
        action = self.max_action * action

        # 计算 tanh-squash 后的 log_prob 修正项
        # 公式: log_prob = log_prob_unbound - sum(log(1 - action^2)) 
        # 但要注意这里 action_unbound 是 pre-activation, action 是 tanh(action_unbound)
        # => 1 - tanh(x)^2 = sech(x)^2
        # => derivative: d(tanh(x))/dx = sech(x)^2
        # 参考 SAC 论文附录
        log_prob_squash = torch.sum(
            torch.log(1 - torch.tanh(action_unbound).pow(2) + 1e-7),
            dim=1,
            keepdim=True
        )
        log_prob = log_prob_unbound - log_prob_squash
...
class SAC(object):
    ...
    def get_action(self, state):
        ...
        action = torch.tanh(action)  # Tanh squashing function to ensure action is within the range
    ...
    discount=0.99,
    tau=0.005,
    policy_freq=1,  # SAC 通常每步都更新 actor
    alpha=0.2,
    actor_lr=5e-6,      # 熵系数
    ):
        critic_lr = 1e-4
       ...
       # actor_lr = 1e-5 # Actor的学习率
        ...
        for it in range(iterations):
            ...
            batch_rewards,
            batch_dones,
            batch_next_states) = replay_buffer.sample_batch(batch_size)
            ...
            # 2) 用 actor_target 来采样下一个动作 & log_prob
            #    (有时 SAC 不一定需要 actor_target，你也可只用 actor本身；但许多实现中同样有 target)
            ...
            with torch.no_grad():
                ...
                # SAC 中的 target: r + gamma * ( Q - alpha * log_prob )
                target_Q = reward + (1 - done) * discount * (q_next - alpha * next_log_prob)
            ...
            c_av_loss += critic_loss.item()

            # 用 min(Q1,Q2) 看看平均 Q 值
            with torch.no_grad():
                min_curQ = torch.min(current_Q1, current_Q2)
                av_Q += min_curQ.mean().item()
                max_Q = max(max_Q, min_curQ.max().item())

            # 5) 每隔 policy_freq 步 更新 Actor
            if it % policy_freq == 0:
                # 5.1 用当前 actor 采样
                cur_action, log_prob = self.actor.sample(state)
                q1_pi, q2_pi = self.critic(state, cur_action)
                q_pi = torch.min(q1_pi, q2_pi)

                # 5.2 Actor Loss: alpha * log_prob - Q
                #    因为要最大化 (Q - alpha*log_prob)，等价于最小化 ( -Q + alpha*log_prob )
        ...
        # 把累计的量做个平均
        c_av_loss /= iterations
        a_av_loss /= iterations
        av_Q /= iterations

        self.iter_count += 1
        Critic_Loss_queue.put((c_av_loss, counter_epoch))
        Actor_Loss_queue.put((a_av_loss, counter_epoch))
        Av_Q_queue.put((av_Q, counter_epoch))
        Max_Q_queue.put((max_Q, counter_epoch))
...
def worker(t,network,counter_epoch,counter_epoch_success,param,success_queue,reward_queue,Critic_Loss_queue,Actor_Loss_queue,Av_Q_queue,Max_Q_queue,succes_all,counter_epoch_all,timestep_worker,expl_noise_episode,reward_all,win,all_step,speed,scale_to_goal,beta,Lamda,beta_Lamda_decay,obs_scale,timestep_episode,logtest,up_speed,smoothness_scale,acceleration_scale,ounoise_decay_step,counter_epoch_collision,collision_queue,counter_epoch_timeout,timeout_queue,step_penalty,W_speed,actor_lr):
   # writer = SummaryWriter(log_dir=f"runs/1worker_, reward14+nopre") 
    ...
    if not os.path.exists("./results"):
        os.makedirs("./results")
    if save_model and not os.path.exists("./pytorch_models/SAC"):
        os.makedirs("./pytorch_models/SAC")
    ...
    while timestep < max_timesteps:
        ...
        if done:
                    ...
                    if timesteps_since_eval >= eval_freq:
                        ...
                        # )
                        network.save(file_name+str(test), directory="./pytorch_models/SAC")
     ...
     #   print(a_in)
        ...
        next_state, reward, done, target = env.step(state[-state_dim],a_in,omega1)
        # 先增加一个新维度，将形状变为 (1, state_dim)
...
if __name__ == '__main__':
    ...
    action_dim = 2
    max_action = 1
    MasterNode = SAC(state_dim, action_dim, max_action) #A
    ...
    logdir = f"./SAC/max_speed_1_collsion_dist_0.45_k_250_threshold_0.45_1/reward+{scale_to_goal}+{obs_scale}+{logtest}+{test}+{up_speed}+{smoothness_scale}+{acceleration_scale}+{ounoise_decay_step}+{step_penalty}+{actor_lr}"
    processes=[]
    params={'n_workers':20,
            'n_loggers':2,
            }
